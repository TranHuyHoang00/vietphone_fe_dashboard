...
const get_list_variant = (date_filter) => {
    return api_admin.get(`/product/api/v1/list-product-variant?page=${date_filter.page}&limit=${date_filter.limit}${(date_filter.search === null || date_filter.search === '') ? '' : `&search=${date_filter.search}`}`);
}
