...
const get_list_task = (date_filter) => {
    return api_admin.get(`/sapo/api/v1/list-task-result?page=${date_filter.page}&limit=${date_filter.limit}${(date_filter.search === null || date_filter.search === '') ? '' : `&search=${date_filter.search}`}`);
}
