...
const get_list_product_page = (date_filter) => {
    return api_admin.get(`/api/v1/list-product-page?page=${date_filter.page}&limit=${date_filter.limit}${(date_filter.search === null || date_filter.search === '') ? '' : `&search=${date_filter.search}`}`);
}
