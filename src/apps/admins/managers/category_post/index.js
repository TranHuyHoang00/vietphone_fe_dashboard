...
import { data_category_posts } from '@datas/data_after_check_permissions';
class index extends Component {
    ...
    async componentDidMount() {
        this.props.get_list_category_post(this.state.data_filter);
        let data_before_checks = await check_permission(data_category_posts, this.props.data_user_permissions, this.props.is_superuser);
        this.setState({
            data_before_checks: data_before_checks,
        });
    }
    open_modal = async (name, value, id) => {
        this.props.set_data_category_post({});
        if (name === 'create') {
            this.setState({ modal_create: value });
        }
        if (name === 'edit') {
            if (id === undefined) {
                this.setState({ modal_edit: value, data_category_post: {} });
            } else {
                this.setState({ modal_edit: value });
                await this.props.get_category_post(id);
            }
        }
    }
    handle_funtion_menu = async () => {
        let data_selected = this.state.data_selected;
        if (this.state.type_menu === 1) { await this.props.delete_list_category_post(data_selected); }
        await this.props.get_list_category_post(this.state.data_filter);
    ...
    onchange_page = async (value, type) => {
        ...
        await this.props.get_list_category_post(data_filter);
    }
    render() {
        const columns = [
            {
            ...
            },
            {
                ...
                render: (_, item) => (
                    <Space size="middle" >
                        <button disabled={!data_before_checks['post.add_category']} onClick={() => this.open_modal('edit', true, item.id)}>
                            <AiFillEdit />
                        </button>
                    </Space >
                ),
        ...
        const items = [
            { key: 1, label: 'Xóa', disabled: !data_before_checks['post.delete_category'] },
        ];
        ...
        return (
            <>
                <Spin size='large' spinning={this.props.is_loading}>
                    <div className="mx-[10px] space-y-[10px]">
                        <div className='flex items-center justify-between gap-[10px]'>
                            <Button disabled={!data_before_checks['post.add_category']}
                        ...
                        <div className='bg-white dark:bg-[#001529] p-[10px] rounded-[10px] shadow-md'>
                            <div className='flex items-center justify-between gap-[10px]'>
                                ...
                                <div>
                                    <Popconfirm disabled={(data_selected && data_selected.length === 0 ? true : false)}
                                        ...
                                        <Dropdown.Button disabled={!data_before_checks['post.delete_category']}
                            ...
                            <div className='space-y-[20px]'>
                                <Table rowSelection={row_selection} rowKey="id"
                                    columns={columns} dataSource={this.props.data_category_posts} pagination={false}
                ...
                {this.state.modal_create && data_before_checks['post.add_category'] &&
                    <ModalCreate modal_create={this.state.modal_create}
                        open_modal={this.open_modal}
                        data_filter={this.state.data_filter} />}
                {this.state.modal_edit && data_before_checks['post.change_category'] &&
...
const mapStateToProps = state => {
    return {
        ...
        data_meta: state.category_post.data_meta,
...
const mapDispatchToProps = dispatch => {
    return {
        ...
        get_category_post: (id) => dispatch(actions.get_category_post_redux(id)),
        edit_list_category_post: (id, data) => dispatch(actions.edit_list_category_post_redux(id, data)),
        delete_list_category_post: (id) => dispatch(actions.delete_list_category_post_redux(id)),
        set_data_category_post: (id) => dispatch(actions.set_data_category_post_redux(id)),
    };
};
