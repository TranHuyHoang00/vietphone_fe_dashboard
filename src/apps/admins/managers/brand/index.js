...
import { data_brands } from '@datas/data_after_check_permissions';
class index extends Component {
    ...
    async componentDidMount() {
        ...
        let data_before_checks = await check_permission(data_brands, this.props.data_user_permissions, this.props.is_superuser);
        this.setState({
    ...
    handle_funtion_menu = async () => {
        ...
        if (this.state.type_menu === 2) { await this.props.edit_list_brand(data_selected, { is_active: false }); }
        if (this.state.type_menu === 3) { await this.props.edit_list_brand(data_selected, { is_active: true }); }
    ...
    onchange_page = async (value, type) => {
        ...
        await this.props.get_list_brand(data_filter);
    }
    render() {
        const columns = [
            {
            ...
            },
            {
                ...
                render: (_, item) => (
                    <Space size="middle" >
                        <button disabled={!data_before_checks['product.view_brand']} onClick={() => this.open_modal('detail', true, item.id)}><AiFillEye /></button>
                        <button disabled={!data_before_checks['product.change_brand']} onClick={() => this.open_modal('edit', true, item.id)}>
                            <AiFillEdit />
                        </button>
                    </Space >
                ),
        ...
        const items = [
            { key: 1, label: 'Xóa', disabled: !data_before_checks['product.delete_brand'] },
            { key: 2, label: 'Khóa', disabled: !data_before_checks['product.change_brand'] },
            { key: 3, label: 'Mở', disabled: !data_before_checks['product.change_brand'] },
        ];
        ...
        return (
            <>
                <Spin size='large' spinning={this.props.is_loading}>
                    <div className="mx-[10px] space-y-[10px]">
                        <div className='flex items-center justify-between gap-[10px]'>
                            <Button disabled={!data_before_checks['product.add_brand']}
                            ...
                            <div><Input.Search onSearch={(value) => this.onchange_page(value, 'search')} placeholder="Tên thương hiệu !" /></div>
                        </div>
                            ...
                            <div className='flex items-center justify-between gap-[10px]'>
                                ...
                                <div>
                                    <Popconfirm disabled={(data_selected && data_selected.length === 0 ? true : false)}
                                        ...
                                        <Dropdown.Button
                                            disabled={!data_before_checks['product.delete_brand']}
                ...
                {this.state.modal_create && data_before_checks['product.add_brand'] &&
                    <ModalCreate modal_create={this.state.modal_create}
                        open_modal={this.open_modal}
                        data_filter={this.state.data_filter} />}
                {this.state.modal_detail && data_before_checks['product.view_brand'] &&
                    <ModalDetail modal_detail={this.state.modal_detail}
                        open_modal={this.open_modal} />}
                {this.state.modal_edit && data_before_checks['product.change_brand'] &&
...
const mapStateToProps = state => {
    return {
        ...
        data_meta: state.brand.data_meta,
