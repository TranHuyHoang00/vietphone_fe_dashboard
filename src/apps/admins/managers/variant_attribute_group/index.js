...
import { data_variant_attribute_groups } from '@datas/data_after_check_permissions';
class index extends Component {
    ...
    async componentDidMount() {
        ...
        let data_before_checks = await check_permission(data_variant_attribute_groups, this.props.data_user_permissions, this.props.is_superuser);
        this.setState({
    ...
    render() {
        const columns = [
            ...
            },
            {
                ...
                render: (_, item) => (
                    <Space size="middle" >
                        <button disabled={!data_before_checks['product.change_variantattributegroup']} className='cursor-pointer' onClick={() => this.open_modal('edit', true, item.id)}>
                            <AiFillEdit />
                        </button>
                    </Space >
                ),
        ...
        const items = [
            { key: 1, label: 'Xóa', disabled: !data_before_checks['product.delete_variantattributegroup'] },
        ];
        ...
        return (
            <>
                <Spin size='large' spinning={this.props.is_loading}>
                    <div className="mx-[10px] space-y-[10px]">
                        <div className='flex items-center justify-between gap-[10px]'>
                            <Button disabled={!data_before_checks['product.add_variantattributegroup']}
                            ...
                            <div className='flex items-center justify-between gap-[10px]'>
                                ...
                                <div>
                                    <Popconfirm disabled={(data_selected && data_selected.length === 0 ? true : false)}
                                        ...
                                        <Dropdown.Button
                                            disabled={!data_before_checks['product.delete_variantattributegroup']}
                ...
                {this.state.modal_create && data_before_checks['product.add_variantattributegroup'] &&
                    <ModalCreate modal_create={this.state.modal_create}
                        open_modal={this.open_modal}
                        data_filter={this.state.data_filter} />}
                {this.state.modal_edit && data_before_checks['product.change_variantattributegroup'] &&
