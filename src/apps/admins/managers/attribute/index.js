...
class index extends Component {
    ...
    async componentDidMount() {
        ...
        const { getListAttribute, dataUserPermis, isSuperUser } = this.props;
        await getListAttribute(dataFilter);
        const dataCheckPermis = await handleCheckPermis(dataAttributes, dataUserPermis, isSuperUser);
        this.setState({
            dataCheckPermis: dataCheckPermis,
        });
    }
    openModal = async (modalName, modalValue, itemId,) => {
        const { setDataAttribute, getDataAttribute } = this.props;
        const actions = {
            setData: setDataAttribute,
            getData: getDataAttribute,
        };
    ...
    funcDropButtonHeaderOfTable = async () => {
        ...
        const { deleteListAttribute, editListAttribute, getListAttribute } = this.props;
        const actions = {
            deleteList: deleteListAttribute,
            editList: editListAttribute,
    ...
    render() {
        const columns = [
            ...
            },
            {
                ...
                render: (_, item) => (
                    <Space size="middle" >
                        <button disabled={!dataCheckPermis['product.change_attribute']} className='cursor-pointer' onClick={() => this.openModal('edit', true, item.id)}>
                            <AiFillEdit />
                        </button>
                    </Space >
                ),
        ...
        const { isLoading, dataAttributes, dataMeta } = this.props;
        const items = [
            { key: 1, label: 'Xóa', disabled: !dataCheckPermis['product.delete_attribute'] },
        ];
        ...
        return (
            <>
                <Spin size='large' spinning={isLoading}>
                    <div className="mx-[10px] space-y-[10px]">
                        <div className='flex items-center justify-between gap-[10px]'>
                            <Button disabled={!dataCheckPermis['product.add_attribute']}
                        ...
                        <div className='bg-white dark:bg-[#001529] p-[10px] rounded-[10px] shadow-md'>
                            <div className='flex items-center justify-between gap-[10px]'>
                                ...
                                <div>
                                    <Popconfirm disabled={(listItemSelected && listItemSelected.length === 0 ? true : false)}
                                        ...
                                        <Dropdown.Button
                                            disabled={!dataCheckPermis['product.delete_attribute']}
                            ...
                            <div className='space-y-[20px]'>
                                <Table rowSelection={rowSelection} rowKey="id"
                                    columns={columns} dataSource={dataAttributes} pagination={false}
                ...
                {modalCreate && dataCheckPermis['product.add_attribute'] &&
                    <ModalCreate modalCreate={modalCreate}
                        openModal={this.openModal}
                        dataFilter={dataFilter} />}
                {modalEdit && dataCheckPermis['product.change_attribute'] &&
...
const mapDispatchToProps = dispatch => {
    return {
        ...
        editListAttribute: (id, data) => dispatch(actions.editListAttributeRedux(id, data)),
        deleteListAttribute: (id) => dispatch(actions.deleteListAttributeRedux(id)),
        setDataAttribute: (id) => dispatch(actions.setDataAttributeRedux(id)),
    };
};
