...
import { data_posts } from '@datas/data_after_check_permissions';
class index extends Component {
    ...
    async componentDidMount() {
        ...
        let data_before_checks = await check_permission(data_posts, this.props.data_user_permissions, this.props.is_superuser);
        this.setState({
    ...
    render() {
        const columns = [
            ...
            },
            {
                ...
                render: (_, item) => (
                    <Space size="middle" >
                        <button disabled={!data_before_checks['post.change_post']} className='cursor-pointer' onClick={() => this.open_modal('edit', true, item.id)}>
                            <AiFillEdit />
                        </button>
                    </Space >
                ),
        ...
        const items = [
            { key: 1, label: 'Xóa', disabled: !data_before_checks['post.delete_post'] },
        ];
        ...
        return (
            <>
                <Spin size='large' spinning={this.props.is_loading}>
                    <div className="mx-[10px] space-y-[10px]">
                        <div className='flex items-center justify-between gap-[10px]'>
                            <Button disabled={!data_before_checks['post.add_post']}
                            ...
                            <div className='flex items-center justify-between gap-[10px]'>
                                ...
                                <div>
                                    <Popconfirm disabled={(data_selected && data_selected.length === 0 ? true : false)}
                                        ...
                                        <Dropdown.Button
                                            disabled={!data_before_checks['post.delete_post']}
                ...
                {this.state.modal_create && data_before_checks['post.add_post'] &&
                    <ModalCreate modal_create={this.state.modal_create}
                        open_modal={this.open_modal}
                        data_filter={this.state.data_filter} />}
                {this.state.modal_edit && data_before_checks['post.change_post'] &&
