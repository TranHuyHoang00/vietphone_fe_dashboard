...
import { data_categorys } from '@datas/data_after_check_permissions';
class index extends Component {
    ...
    async componentDidMount() {
        ...
        let data_before_checks = await check_permission(data_categorys, this.props.data_user_permissions, this.props.is_superuser);
        this.setState({
    ...
    render() {
        const columns = [
            ...
            },
            {
                ...
                render: (_, item) => (
                    <Space size="middle" >
                        <button disabled={!data_before_checks['product.view_category']} onClick={() => this.open_modal('detail', true, item.id)}><AiFillEye /></button>
                        <button disabled={!data_before_checks['product.change_category']} onClick={() => this.open_modal('edit', true, item.id)}>
                            <AiFillEdit />
                        </button>
                    </Space >
                ),
        ...
        const items = [
            { key: 1, label: 'Xóa', disabled: !data_before_checks['product.delete_category'] },
            { key: 2, label: 'Khóa', disabled: !data_before_checks['product.change_category'] },
            { key: 3, label: 'Mở', disabled: !data_before_checks['product.change_category'] },
        ];
        ...
        return (
            <>
                <Spin size='large' spinning={this.props.is_loading}>
                    <div className="mx-[10px] space-y-[10px]">
                        <div className='flex items-center justify-between gap-[10px]'>
                            <Button disabled={!data_before_checks['product.add_category']}
                            ...
                            <div className='flex items-center justify-between gap-[10px]'>
                                ...
                                <div>
                                    <Popconfirm disabled={(data_selected && data_selected.length === 0 ? true : false)}
                                        ...
                                        <Dropdown.Button
                                            disabled={!data_before_checks['product.delete_category']}
                ...
                {this.state.modal_create && data_before_checks['product.add_category'] &&
                    <ModalCreate modal_create={this.state.modal_create}
                        open_modal={this.open_modal}
                        data_filter={this.state.data_filter} />}
                {this.state.modal_detail && data_before_checks['product.view_category'] &&
                    <ModalDetail modal_detail={this.state.modal_detail}
                        open_modal={this.open_modal} />}
                {this.state.modal_edit && data_before_checks['product.change_category'] &&
