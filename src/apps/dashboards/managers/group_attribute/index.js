...
import { get_list_group_attribute, get_group_attribute, delete_group_attribute } from '../../../../services/group_attribute_service';
...
class index extends Component {
    ...
    get_group_attribute = async (id) => {
        this.handle_loading(true);
        try {
            let data = await get_group_attribute(id);
            if (data && data.data && data.data.success == 1) {
                this.setState({ data_group_attribute: data.data.data });
            } else {
                message.error("Lỗi");
            }
        } catch (e) {
    ...
    open_modal = async (name, value, id) => {
        ...
        if (name == 'detail') {
            if (id == null) {
                ...
                await this.get_group_attribute(id);
            }
        }
        if (name == 'edit') {
            if (id == null) {
    ...
    handle_funtion_menu = async () => {
        ...
        try {
            ...
            for (const id of data_selected) {
                let data;
                if (this.state.type_menu == 1) { data = await delete_group_attribute(id); }
                if (data && data.data && data.data.success !== 1) {
    ...
    onchange_page = async (value, type) => {
        ...
        if (type == 'limit') {
            this.props.history.push(`/admin/manager/group_attribute?page=${data_filter.page}&limit=${value}&search_query=${data_filter.search_query}`);
        }
        if (type == 'page') {
            this.props.history.push(`/admin/manager/group_attribute?page=${value}&limit=${data_filter.limit}&search_query=${data_filter.search_query}`);
        }
    }
    on_search = async (value) => {
        let data_filter = this.state.data_filter;
        this.props.history.push(`/admin/manager/group_attribute?page=1&limit=${data_filter.limit}&search_query=${value}`);
    }
    render() {
        const columns = [
            {
        ...
        return (
            <>
                <Spin size='large' spinning={this.state.is_loading}>
                    <div className="mx-[10px] space-y-[10px]">
                        <div className='flex items-center justify-between gap-[10px]'>
                            ...
                            <div><Input.Search onSearch={(value) => this.on_search(value)} placeholder="Loại thông số !" /></div>
                        </div>
                        <div className='bg-white p-[10px] rounded-[10px] shadow-sm border'>
                            ...
                            <Divider>LOẠI THÔNG SỐ</Divider>
                            <div className='space-y-[20px]'>
                                <Table rowSelection={row_selection} rowKey="id"
                                    columns={columns} dataSource={this.state.data_group_attributes} pagination={false}
                ...
                <Modal_detail modal_detail={this.state.modal_detail}
                    open_modal={this.open_modal} data_group_attribute={this.state.data_group_attribute} />
                <Modal_edit modal_edit={this.state.modal_edit}
                    open_modal={this.open_modal} load_data={this.load_data}
                    data_group_attribute={this.state.data_group_attribute} />
            </>
        );
    }
