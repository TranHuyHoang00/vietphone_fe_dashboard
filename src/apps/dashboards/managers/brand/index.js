...
class index extends Component {
    ...
    onchange_page = async (value, type) => {
        ...
        if (type == 'limit') {
            this.props.history.push(`/admin/manager/brand?page=${data_filter.page}&limit=${value}&search_query=${data_filter.search_query}`);
        }
        if (type == 'page') {
            this.props.history.push(`/admin/manager/brand?page=${value}&limit=${data_filter.limit}&search_query=${data_filter.search_query}`);
        }
    }
    on_search = async (value) => {
        let data_filter = this.state.data_filter;
        this.props.history.push(`/admin/manager/brand?page=1&limit=${data_filter.limit}&search_query=${value}`);
    }
    render() {
        const columns = [
            {
